...
//      (increment xgpt_count or iris_count, update last_alert_date).
//
// This means watchlist counters only increment for genuinely new tickers,
// and re-running the workflow any number of times is always safe.
//
...
export default defineComponent({
  async run({ steps, $ }) {
    ...
    for (const msg of messages) {
      ...
      console.log(`[parse_and_write] ${emailId} (${scanner.toUpperCase()}): ${tickers.length} ticker(s)`);
      ...
      for (const { symbol, score, action } of tickers) {
        ...
        // ── 2. Alert was new — upsert watchlist and increment counter ─────────
        ...
        console.log(`[parse_and_write] Wrote ${sym} (${scanner}) from ${emailId}`);
      }
    }
